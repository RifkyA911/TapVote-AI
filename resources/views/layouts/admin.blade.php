...
<body class="min-h-full bg-slate-100 text-slate-800 font-sans antialiased flex flex-col md:flex-row relative">

    <!-- Sidebar Navigation -->
    <aside class="relative z-20 w-full md:w-64 bg-white border-r border-slate-200 flex flex-col shrink-0">
        <div class="p-5 flex items-center justify-between border-b border-slate-200">
            <a href="{{ route('admin.dashboard') }}" class="flex items-center space-x-3">
                ...
                <div>
                    <h1 class="font-extrabold text-lg leading-tight text-slate-900">TapVote AI</h1>
                    <p class="text-xs text-blue-700 font-bold tracking-wide">Admin Koperasi</p>
                </div>
            </a>
        </div>
        ...
        <nav class="flex-1 p-4 space-y-1.5 overflow-y-auto">
            ...
            <a href="{{ route('admin.dashboard') }}" class="flex items-center space-x-3 px-3.5 py-2.5 rounded-xl font-bold text-sm transition {{ request()->routeIs('admin.dashboard') ? 'bg-blue-600 text-white shadow-sm' : 'text-slate-600 hover:text-slate-900 hover:bg-slate-100' }}">
                ...
                <span>Live Dashboard Realtime</span>
            </a>
            ...
            <div class="text-[11px] uppercase tracking-wider text-slate-400 font-bold px-3 pt-4 pb-1">Laporan & Hasil</div>
        ...
        <div class="p-4 border-t border-slate-200 space-y-2 bg-slate-50">
            <a href="{{ route('voter.tap') }}" target="_blank" class="w-full flex items-center justify-center space-x-2 py-2.5 px-3 rounded-xl bg-blue-600 hover:bg-blue-700 text-xs font-bold text-white shadow-sm transition">
            ...
            <div class="flex items-center justify-between pt-2">
                <div class="text-xs text-slate-600 truncate pr-2">
                    <span class="block text-slate-900 font-bold">{{ auth()->user()->name }}</span>
                    <span class="text-[11px] text-slate-500">Administrator</span>
                </div>
                <form action="{{ route('admin.logout') }}" method="POST">
                    @csrf
                    <button type="submit" title="Keluar / Logout" class="p-2 rounded-lg bg-rose-50 text-rose-700 hover:bg-rose-600 hover:text-white transition">
    ...
    <div class="flex-1 flex flex-col min-w-0 overflow-y-auto">
        <!-- Top App Bar -->
        <header class="relative z-10 h-16 bg-white border-b border-slate-200 px-6 flex items-center justify-between shadow-sm">
            <div class="flex items-center space-x-3">
                ...
                <span class="text-xs text-slate-500 hidden sm:inline">Database: <strong class="text-slate-800 font-mono">tapvote_ai</strong></span>
            </div>
        ...
        @if(session('success'))
            <div class="m-6 mb-0 p-4 rounded-xl bg-emerald-50 border-2 border-emerald-400 text-emerald-900 text-sm font-bold flex items-center space-x-3 shadow-sm">
        ...
        @if(session('error') || $errors->any())
            <div class="m-6 mb-0 p-4 rounded-xl bg-rose-50 border-2 border-rose-400 text-rose-900 text-sm font-bold flex items-center space-x-3 shadow-sm">
