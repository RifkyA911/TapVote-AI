...
@section('title', 'Supervisor Candidates - TapVote AI')
...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
            ...
            <h2 class="text-2xl font-extrabold text-slate-900">Supervisor Candidates Management</h2>
        ...
        <div class="flex flex-wrap items-center gap-2">
            ...
            <a href="{{ route('admin.pengawas.create') }}" class="px-4 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold shadow-xs transition flex items-center space-x-1.5 cursor-pointer">
                ...
                <span>Add Supervisor Candidate</span>
            </a>
        </div>
    </div>
    ...
    @php
        $totalPengawasVotes = $kandidat->sum('perolehan_suara_count');
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <div class="p-4 rounded-2xl bg-white border border-slate-200 shadow-2xs flex items-center justify-between">
            <div>
                <span class="text-xs text-emerald-700 font-semibold">Total Votes Received</span>
                <strong class="block text-2xl font-black text-emerald-600 font-mono">{{ $totalPengawasVotes }}</strong>
            </div>
            <span class="p-2.5 rounded-xl bg-emerald-50 text-emerald-600">
                        ...
                        <tr draggable="true" class="hover:bg-slate-50/80 transition candidate-row cursor-grab active:cursor-grabbing group" data-nik="{{ $p->nik }}" data-name="{{ strtolower($p->nama) }}" data-visi="{{ strtolower($p->visi) }}">
                            ...
                            <td class="py-3 px-3 text-center">
                                <button 
                                    ...
                                    class="group relative w-14 h-18 rounded-xl overflow-hidden bg-slate-900 border-2 border-slate-200 hover:border-emerald-500 transition-all duration-300 shadow-2xs hover:shadow-md cursor-pointer inline-block"
                            ...
                            <!-- Candidate Identity -->
                            <td class="py-3 px-3">
                                <div class="font-extrabold text-slate-900 text-sm sm:text-base">{{ $p->nama }}</div>
                                <div class="flex items-center space-x-2 mt-1">
                                    <span class="inline-flex items-center px-2 py-0.5 rounded-md bg-emerald-50 text-emerald-700 text-[11px] font-mono font-bold border border-emerald-200">
                                        NIK: {{ $p->nik }}
                                    </span>
                                </div>
                            </td>
                            ...
                            <td class="py-3 px-3 max-w-xs sm:max-w-md">
                                <div class="text-xs text-slate-700 font-semibold line-clamp-2" title="{{ $p->visi }}">
                                    <strong class="text-slate-900">Vision:</strong> {{ $p->visi }}
                                </div>
                                <div class="text-[11px] text-slate-500 line-clamp-1 mt-1">
                                    <strong class="text-slate-700">Mission:</strong> {{ $p->misi }}
                                </div>
                            </td>
                            ...
                            <td class="py-3 px-3 text-center">
                                ...
                                <div class="w-full bg-slate-100 rounded-full h-2 mt-1.5 overflow-hidden">
                                    <div class="bg-emerald-600 h-2 rounded-full transition-all duration-500" style="width: {{ $pct }}%;"></div>
                                </div>
                            </td>
                            ...
                            <td class="py-3 px-3 text-right">
                                <div class="flex items-center justify-end space-x-1.5">
                                    ...
                                    <form action="{{ route('admin.pengawas.destroy', $p->nik) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete Supervisor candidate {{ $p->nama }}?')">
                                        @csrf
                                        @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="py-12 text-center text-slate-400">
                                No Supervisor candidates found. Click "Add Supervisor Candidate" to register.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
