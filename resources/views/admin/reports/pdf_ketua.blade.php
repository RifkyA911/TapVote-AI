...
<head>
    <meta charset="UTF-8">
    <title>Official Report - Chairman Election Results - TapVote AI</title>
    <style>
        ...
        .badge-leader { background-color: #fee2e2; color: #991b1b; }
...
<body>
    ...
    <table class="header-table">
        <tr>
            <td class="header-title">
                ...
                <h2>LAPORAN RESMI PEROLEHAN SUARA CALON KETUA KOPERASI</h2>
    ...
    <div class="doc-title">
        <h3>BERITA ACARA PEROLEHAN SUARA KETUA</h3>
        <p>Nomor Dokumen: BA-KETUA/{{ date('Y/m') }}/{{ strtoupper(substr(md5($totalSuara . 'ketua'), 0, 8)) }} • Tanggal: {{ date('d F Y') }}</p>
    </div>

    <p style="font-size: 9pt; margin-bottom: 8px;">
        Pada hari ini, <strong>{{ date('d F Y') }}</strong>, sistem pemungutan suara digital TapVote AI mencatat total perolehan suara sah calon Ketua Koperasi sejumlah <strong>{{ $totalSuara }} suara</strong> dengan rincian sebagai berikut:
    </p>
    ...
    <table class="table-data">
        ...
        <tbody>
            @foreach($kandidatList as $k)
                ...
                @endphp
                <tr>
                    <td style="text-align: center; font-weight: bold;">{{ $k->nomor_urut }}</td>
                    <td style="font-family: monospace;">{{ $k->nik }}</td>
                    <td style="font-weight: bold;">{{ $k->nama }}</td>
                    <td style="text-align: center; font-weight: bold;">{{ $k->perolehan_suara_count }}</td>
