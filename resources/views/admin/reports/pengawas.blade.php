...
@section('title', 'Supervisor Election Recap & Results')
...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
            <div class="flex items-center space-x-2 mb-1">
                <span class="px-2.5 py-0.5 rounded-lg text-[10px] font-black uppercase tracking-widest bg-emerald-100 text-emerald-800 border border-emerald-200">
                    Supervisor Recap
                </span>
            ...
            <h2 class="text-2xl font-extrabold text-slate-900">Hasil & Rekapitulasi Pengawas Koperasi</h2>
            <p class="text-xs sm:text-sm text-slate-500 mt-0.5">Perolehan suara resmi, penetapan calon pengawas terpilih, dan rincian suara per kandidat.</p>
        </div>

        <div class="flex flex-wrap items-center gap-2">
            <a href="{{ route('admin.reports.pengawas.export') }}" class="px-3.5 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold shadow-xs transition flex items-center space-x-1.5 cursor-pointer">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg>
                <span>Export Excel</span>
            </a>

            <a href="{{ route('admin.reports.pengawas.export.pdf') }}" class="px-3.5 py-2 rounded-xl bg-white hover:bg-slate-100 text-slate-700 text-xs font-bold border border-slate-300 shadow-2xs transition flex items-center space-x-1.5 cursor-pointer" title="Download Official Vector PDF">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <div class="p-4 rounded-2xl bg-white border border-slate-200 shadow-2xs flex items-center justify-between">
            <div>
                <span class="text-xs {{ $isSeri ? 'text-amber-700' : 'text-emerald-700' }} font-semibold">Status Hasil Pemilihan</span>
                <strong class="block text-base font-black {{ $isSeri ? 'text-amber-600' : 'text-emerald-600' }} truncate mt-1">
                    {{ $isSeri ? '⚖️ HASIL SERI (DRAW)' : ($pemenang ? '🏆 ADA PEMENANG' : 'BELUM ADA SUARA') }}
                </strong>
            </div>
            <span class="p-2.5 rounded-xl {{ $isSeri ? 'bg-amber-50 text-amber-600' : 'bg-emerald-50 text-emerald-600' }}">
        ...
        <div class="p-6 sm:p-7 rounded-3xl bg-amber-50 border-2 border-amber-300 shadow-2xs relative overflow-hidden">
            <div class="flex flex-col md:flex-row items-start gap-5">
                ...
                <div class="flex-1">
                    ...
                    <h3 class="text-xl sm:text-2xl font-black text-slate-900">Belum Ada Pemenang Tunggal Pengawas Koperasi</h3>
                        ...
                        @foreach($topCandidates as $c)
                            <div class="p-3.5 rounded-2xl bg-white border border-amber-200 shadow-2xs flex items-center space-x-3">
                                <div class="w-12 h-12 rounded-xl overflow-hidden bg-slate-100 border border-amber-300 shrink-0">
                                    <img src="{{ $c->foto ?: 'https://ui-avatars.com/api/?name='.urlencode($c->nama).'&background=059669&color=ffffff&size=200' }}" alt="{{ $c->nama }}" class="w-full h-full object-cover object-top">
                                </div>
                                <div class="min-w-0">
    ...
    @elseif($pemenang && $pemenang->perolehan_suara_count > 0)
        ...
        @endphp
        <div class="p-6 sm:p-7 rounded-3xl bg-emerald-50/80 border-2 border-emerald-200 shadow-2xs relative overflow-hidden">
            <div class="flex flex-col sm:flex-row items-center gap-6 relative z-10">
                <div class="relative">
                    <div class="w-28 h-28 rounded-2xl overflow-hidden bg-white border-2 border-emerald-500 shadow-md">
                        <img 
                    ...
                    <span class="absolute -bottom-2 -right-2 px-2.5 py-0.5 rounded-full bg-emerald-600 text-white font-mono font-bold text-xs shadow-md">
        ...
        <div id="filter-body-container" class="space-y-3.5 transition-all duration-300">
            ...
            <div class="flex items-center gap-3">
                <div class="relative flex-1">
                    <input 
                        ...
                        class="w-full pl-10 pr-10 py-3 rounded-2xl bg-slate-50/80 border border-slate-200 text-xs sm:text-sm text-slate-900 placeholder-slate-400 focus:bg-white focus:border-emerald-500 focus:ring-4 focus:ring-emerald-500/10 outline-none transition font-medium"
                    >
            ...
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-2.5">
                <div class="col-span-1">
                    <select id="candidate-status-filter" onchange="filterCandidateTable()" class="w-full h-11 px-3 py-2 rounded-xl bg-slate-50 border border-slate-200 text-xs text-slate-700 font-bold focus:bg-white focus:border-emerald-500 outline-none cursor-pointer">
                ...
                <div class="col-span-1">
                    <select id="candidate-sort-select" onchange="sortCandidateTable(this.value)" class="w-full h-11 px-3 py-2 rounded-xl bg-slate-50 border border-slate-200 text-xs text-slate-700 font-bold focus:bg-white focus:border-emerald-500 outline-none cursor-pointer">
                ...
                <div class="col-span-1">
                    <button 
                        ...
                        class="w-full h-11 px-4 rounded-xl bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-700 hover:to-teal-700 text-white text-xs font-black shadow-xs transition flex items-center justify-center space-x-1.5 cursor-pointer"
                ...
                <tbody class="divide-y divide-slate-100" id="candidate-table-body">
                    @foreach($kandidatPengawas as $p)
                        ...
                        <tr class="hover:bg-slate-50/75 transition border-b border-slate-100 candidate-row" 
                            data-nomor="{{ $p->nomor_urut }}"
                            data-nama="{{ strtolower($p->nama) }}"
                            data-nik="{{ $p->nik }}"
                            data-votes="{{ $p->perolehan_suara_count }}"
                            ...
                            <td class="py-3 px-4">
                                <div class="flex items-center space-x-3">
                                    <div class="w-10 h-10 rounded-xl overflow-hidden bg-slate-100 border border-slate-200 shrink-0">
                                        <img src="{{ $p->foto ?: 'https://ui-avatars.com/api/?name='.urlencode($p->nama).'&background=059669&color=ffffff&size=100' }}" alt="{{ $p->nama }}" class="w-full h-full object-cover object-top">
                                    </div>
                                    <div>
                                        <strong class="font-extrabold text-slate-900 block">{{ $p->nama }}</strong>
                                        <span class="text-xs text-slate-400">Calon Pengawas Koperasi</span>
                                    </div>
                                </div>
                            </td>
                            <td class="py-3 px-4 text-slate-500 font-mono">{{ $p->nik }}</td>
                            <td class="py-3 px-4 text-right">
                                <strong class="font-black text-slate-900 font-mono text-sm">{{ $p->perolehan_suara_count }}</strong>
                                <span class="text-xs text-slate-400 ml-0.5">Suara</span>
                            </td>
                            <td class="py-3 px-4 text-right">
                                <span class="font-mono font-black text-emerald-600 text-sm">{{ $persen }}%</span>
